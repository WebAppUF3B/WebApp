 ...
 * Globals
 */
let user1, admin1, userFromSeedConfig, adminFromSeedConfig, originalLogConfig;
...
describe('Configuration Tests:', () => {
  ...
  describe('Testing default seedDB', () => {
    ...
    it('should seed ONLY the admin user account when NODE_ENV is set to "production"', (done) => {
      ...
      User.find({ username: adminFromSeedConfig.username }, (err, users) => {
        ...
        seed
          ...
          .then(() => {
            User.find({ username: adminFromSeedConfig.username }, (err, users) => {
              ...
              User.find({ username: userFromSeedConfig.username }, (err, users) => {
    ...
    it('should seed admin, and "regular" user accounts when NODE_ENV is set to "test" when they already exist', (done) => {
      ...
      _admin.save((err) => {
        ...
        _user.save((err) => {
          ...
          User.find({ username: { $in: [adminFromSeedConfig.username, userFromSeedConfig.username] } }, (err, users) => {
    ...
    it('should ONLY seed admin user account when NODE_ENV is set to "production" with custom admin', (done) => {
      ...
      User.find({ username: admin1.username }, (err, users) => {
        ...
        seed
          ...
          .then(() => {
            User.find({ username: admin1.username }, (err, users) => {
              ...
              User.find({ username: user1.username }, (err, users) => {
  ...
  describe('Testing Logger Configuration', () => {
    ...
    it('should retrieve the log format from the logger configuration', () => {
      ...
      const format = logger.getFormat();
      format.should.be.equal('tiny');
    });

    it('should retrieve the log options from the logger configuration', () => {
      config.log = {
        options: {
    ...
    it('should verify that a writable stream was created using the logger configuration', () => {
    ...
    it('should remove the stream option when an invalid filename was supplied for the log stream option', () => {
    ...
    it('should remove the stream option when an invalid directory path was supplied for the log stream option', () => {
      // manually set the config stream fileName option to an empty string
      config.log = {
        format: 'combined',
        options: {
    ...
    it('should confirm that the log directory is created if it does not already exist', () => {
      const _dir = process.cwd() + '/temp-logs';
      const _filename = 'unit-test-access.log';
    ...
    it('should remove the stream option when an invalid filename was supplied for the rotating log stream option', () => {
    ...
    it('should confirm that the rotating log is created using the logger configuration', () => {
      const _dir = process.cwd();
      const _filename = 'unit-test-rotating-access-%DATE%.log';
    ...
    it('should confirm that the rotating log directory is created if it does not already exist', () => {
      const _dir = process.cwd() + '/temp-rotating-logs';
      const _filename = 'unit-test-rotating-access-%DATE%.log';
