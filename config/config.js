...
const initGlobalConfig = function () {
  ...
  const defaultAssets = require(path.join(process.cwd(), 'config/assets/default')); // eslint-disable-line global-require

  // Get the current assets
  const environmentAssets = require(path.join(process.cwd(), 'config/assets/', process.env.NODE_ENV)) || {}; // eslint-disable-line global-require

  // Merge assets
  const assets = _.merge(defaultAssets, environmentAssets);

  // Get the default config
  const defaultConfig = require(path.join(process.cwd(), 'config/env/default')); // eslint-disable-line global-require

  // Get the current config
  const environmentConfig = require(path.join(process.cwd(), 'config/env/', process.env.NODE_ENV)) || {}; // eslint-disable-line global-require

  // Merge config files
  let config = _.merge(defaultConfig, environmentConfig);

  // read package.json for MEAN.JS project information
  const pkg = require(path.resolve('./package.json')); // eslint-disable-line global-require
  config.meanjs = pkg;
